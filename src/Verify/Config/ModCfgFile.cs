...
internal static class ModCfgFile
{
    ...
    internal static void Init(Mod mod)
    {
        try
        {
            ...
            _path = Path.Combine(dir, "companion_config.txt");
    ...
    private static bool Apply(string key, string val)
    {
        switch (key)
        {
            ...
            case "DebugOverlayHeight": return TryF(val, ref Cfg.DebugOverlayHeight);
            case "DebugOverlayWidth": return TryF(val, ref Cfg.DebugOverlayWidth);
            case "DebugOverlayAutoShrink": return TryBool(val, ref Cfg.DebugOverlayAutoShrink);
            case "DebugOverlayShrinkNearM": return TryF(val, ref Cfg.DebugOverlayShrinkNearM);
            case "DebugOverlayConnectWaypoints": return TryBool(val, ref Cfg.DebugOverlayConnectWaypoints);
            // ★ [jump] 障害物ジャンプ（v0.8.1）
            ...
            case "WeaponClassifyMode":
            {
                ...
                if (m == "auto" || m == "strict")
                {
                    Cfg.WeaponClassifyMode = m;
                    return true;
                }

                return false;
            }
            case "MeleeWeaponNames":
                Cfg.MeleeWeaponNames = val;
                return true;
            case "RangedWeaponNames":
                Cfg.RangedWeaponNames = val;
                return true;
            case "WeaponExcludeNames":
                Cfg.WeaponExcludeNames = val;
                return true;
            case "MeleeWeaponTags":
                Cfg.MeleeWeaponTags = val;
                return true;
            case "RangedWeaponTags":
                Cfg.RangedWeaponTags = val;
                return true;
            case "WeaponExcludeTags":
                Cfg.WeaponExcludeTags = val;
                return true;
    ...
    private static string DefaultText()
    {
        return
            ...
            $"DebugOverlay                  = false\n" +
            $"# 柱のサイズ（大きすぎる場合はここを下げる）\n" +
            $"DebugOverlayHeight            = 2.0\n" +
            $"DebugOverlayWidth             = 0.12\n" +
            $"# 自分↔リーダーが近いほど柱を自動縮小（最小50%）。NearM 以下で縮小開始し 0m で半分\n" +
            $"DebugOverlayAutoShrink        = true\n" +
            $"DebugOverlayShrinkNearM       = 4.0\n" +
            $"# WP柱を配列順(経路順)に直線で結ぶ\n" +
            $"DebugOverlayConnectWaypoints  = true\n";
    }
}