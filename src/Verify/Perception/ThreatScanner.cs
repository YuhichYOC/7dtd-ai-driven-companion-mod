...
internal static class ThreatScanner
{
    ...
    internal static ThreatInfo ScanNearestActiveThreat(World world, EntityPlayerLocal self)
    {
        ...
        var r = Cfg.ThreatScanRadius;
        var box = new Bounds(self.position, new Vector3(r * 2f, r * 2f, r * 2f));

        var found = world.GetLivingEntitiesInBounds(self, box);
        ...
        for (var i = 0; i < found.Count; i++)
        {
            ...
            var d = e.position - self.position;
            var dSq = d.sqrMagnitude;
            if (dSq > rSq) continue;

            LastHostileCount++;

            var st = GetAwareness(e, self);
            if (st == Awareness.Unawakened)
            {
                LastSleepingCount++;
                continue;
            }
    ...
    private static Awareness GetAwareness(EntityAlive e, EntityPlayerLocal self)
    {
        ...
        var tgt = e.GetAttackTargetLocal(); // remote時 attackTargetClient
