...
internal static class ItemStower
{
    ...
    private static void Run(EntityPlayerLocal self)
    {
        ...
        for (var i = 0; i < bslots.Length; i++)
        {
            ...
            if (inv.AddItem(st.Clone(), out var slot) && slot >= 0)
            {
    ...
    {
        var ic = st.itemValue.ItemClass;
        var nm = ic != null && ic.Name != null ? ic.Name : "type" + st.itemValue.type;
