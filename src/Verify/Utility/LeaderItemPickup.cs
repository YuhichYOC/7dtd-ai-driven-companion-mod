        ...
        internal static void MaybeRun(EntityPlayerLocal self, EntityPlayer leader)
        {
            ...
            for (int i = 0; i < _buf.Count; i++)
            {
                ...
                if (leaderOwned || freeToGrab)
                {
                    ...
                    collected++;
                    Log.Out($"[CompanionAI] pickup: collect id={e.entityId} owner={e.belongsPlayerId} d={Mathf.Sqrt(dSq):0.0}m ({(leaderOwned ? "leader" : "unowned")})");
                }
            }
            ...
            if (seen > 0 && collected == 0 && Time.time >= _nextLog)
            {
                ...
                Log.Out($"[CompanionAI] pickup: {seen} item(s) in range, none matched (firstOwner={firstOwner}, leaderId={leader.entityId}, selfId={self.entityId}).");
            }
        }
    }
}
