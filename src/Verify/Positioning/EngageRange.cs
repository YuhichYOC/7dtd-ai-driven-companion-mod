...
namespace CompanionAIVerify.Positioning
{
    ...
    ///  - 主攻撃アクションは Actions[0]。         Inventory.GetHoldingGun(): holdingItem.Actions[0] as ItemActionAttack   (Inventory.cs:737-739)
    ///                                            GetPrimaryAction(): holdingItem.Actions[0]                              (Inventory.cs:729)
    ///  - 遠隔/近接の判別も Actions[0] で行う。   vanilla: _itemValue.ItemClass.Actions[0] is ItemActionRanged            (ItemActionAttack.cs:1064)
    ///  - 遠隔の実効射程 = MaxRange 適用後。      ItemActionRanged.GetRange(): EffectManager.GetValue(MaxRange, .., Range) (ItemActionRanged.cs:1376-1378)
    ///  - 近接のリーチ = Range フィールド。       melee hit は distanceSq > Range*Range で棄却                            (ItemActionMelee.cs:148,228)
    ...
    internal static class EngageRange
    {
        ...
        internal static readonly Info Invalid = new Info { valid = false };
        ...
        internal static Info Read(EntityAlive holder)
        {
            ...
            Inventory inv = holder.inventory;                       // EntityAlive.cs:764
            if (inv == null) return Invalid;

            ItemClass held = inv.holdingItem;                       // Inventory.cs:182
            ItemInventoryData heldData = inv.holdingItemData;       // Inventory.cs:208
            if (held == null || heldData == null) return Invalid;
            if (held.Actions == null || held.Actions.Length == 0) return Invalid;

            ItemAction a0 = held.Actions[0];                        // 主攻撃アクション（Inventory.cs:729,737-739）
            ...
            ItemActionRanged ranged = a0 as ItemActionRanged;
            ...
            ItemActionDynamic dyn = a0 as ItemActionDynamic;
            if (dyn != null)
            {
                info.isRanged = false;
            ...
            ItemActionAttack atk = a0 as ItemActionAttack;
            if (atk != null)
            {
                info.isRanged = false;
        ...
        internal static float EyeToChestDistance(EntityAlive holder, EntityAlive target)
        {
            ...
            Vector3 eye = holder.GetLookRay().origin;
            Vector3 chest = target.getChestPosition();
        ...
        struct LogState { public float t; public string key; }
        static readonly Dictionary<int, LogState> _last = new Dictionary<int, LogState>();
        ...
        internal static void LogTick(EntityAlive holder, EntityAlive target)
        {
            ...
            Info info = Read(holder);

            float d = (target != null) ? EyeToChestDistance(holder, target) : float.NaN;
            bool inRange = info.valid && target != null && d <= info.range;

            // 変更検知キー（丸めてチラつきを抑える）
            string key = info.valid
                ? $"{(info.isRanged ? "RANGED" : "MELEE")}|R={info.range.ToString("F2")}|d={(float.IsNaN(d) ? "na" : d.ToString("F1"))}|in={(inRange ? "1" : "0")}"
                : "INVALID";

            int id = holder.entityId;
            float now = Time.time;
            if (_last.TryGetValue(id, out LogState st))
            {
                if (st.key == key && (now - st.t) < Cfg.EngageLogMinInterval) return;
            }
            _last[id] = new LogState { t = now, key = key };

            if (!info.valid)
            {
                Log.Out("[CompanionAI][EngageRange] eid={0} weapon=INVALID (no attack action / bare hand)", id);
                return;
            }

            Log.Out($"[CompanionAI][EngageRange] eid={id} {(info.isRanged ? "RANGED" : "MELEE")} action={info.actionType} range={info.range:F2} " +
